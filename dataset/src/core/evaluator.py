...
class PointCloudEvaluator:
    ...
    def _nearest_distances(source: np.ndarray, target: np.ndarray) -> np.ndarray:
        """Compute nearest neighbor distances from source to target using KDTree.
        
        This is memory-efficient O(N log M) instead of O(N*M) with cdist.
        
        Args:
            source: Source points (N x 3)
            target: Target points (M x 3)
            
        Returns:
            Array of nearest distances for each source point (N,)
        """
        tree = cKDTree(target)
    ...
    def chamfer_distance(pred_points: np.ndarray, gt_points: np.ndarray) -> Tuple[float, float, float]:
        ...
        distances_pred_to_gt = PointCloudEvaluator._nearest_distances(pred_points, gt_points)
        pred_to_gt = float(distances_pred_to_gt.mean())
        
        # Distance from ground truth to predicted
        distances_gt_to_pred = PointCloudEvaluator._nearest_distances(gt_points, pred_points)
        gt_to_pred = float(distances_gt_to_pred.mean())
    ...
    def hausdorff_distance(pred_points: np.ndarray, gt_points: np.ndarray) -> float:
        ...
        return float(max(distances_pred_to_gt.max(), distances_gt_to_pred.max()))
    ...
    def completeness_accuracy(pred_points: np.ndarray, gt_points: np.ndarray, 
        ...
        distances_pred_to_gt = PointCloudEvaluator._nearest_distances(pred_points, gt_points)
        accuracy = float((distances_pred_to_gt < threshold).sum() / len(pred_points) * 100)
        
        # Completeness: percentage of GT points within threshold of any predicted point
        distances_gt_to_pred = PointCloudEvaluator._nearest_distances(gt_points, pred_points)
        completeness = float((distances_gt_to_pred < threshold).sum() / len(gt_points) * 100)
