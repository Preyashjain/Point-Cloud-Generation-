...
class BatchReconstructionPipeline:
    ...
    def __init__(self, dataset_root: str, output_root: str, 
                 max_frames: int = 50, log_file: str = None):
        """Initialize pipeline
    ...
    def process_part(self, part_code: str, complexity_level: str = '1_single',
        ...
        try:
            ...
            for orientation, paths in part_data['orientations'].items():
                try:
                    ...
                    self.logger.error(f"Failed to process {part_code}/{orientation}: {e}")
            ...
            # Aggregate results
            successful = sum(1 for o in results['orientations'].values() 
    ...
    def _process_orientation(self, part_code: str, orientation: str, 
                            paths: Dict, save_outputs: bool) -> Dict:
        ...
        if save_outputs:
            try:
                # Point cloud
                pcd_path = self.output_dirs['point_clouds'] / \
                ...
                # Metrics
                metrics_path = self.output_dirs['evaluations'] / \
    ...
    def batch_process(self, part_codes: List[str] = None, 
                     complexity_level: str = '1_single') -> Dict:
    ...
    def save_batch_report(self, batch_results: Dict, output_path: str) -> None:
        ...
        report = {
            'summary': {
                ...
                'successful': sum(1 for p in batch_results['parts'].values() 
                                if p['status'] == 'success'),
                'failed': sum(1 for p in batch_results['parts'].values() 
                            if p['status'] == 'failed')
            },
...
def main():
    ...
    parser.add_argument('--dataset', default='/Users/preyashjain/Downloads/Use_Case_2_Point_Cloud_Generation_dataset',
                       help='Dataset root directory')
    parser.add_argument('--output', default='/Users/preyashjain/Downloads/Use_Case_2_Point_Cloud_Generation_dataset/outputs',
                       help='Output root directory')
    parser.add_argument('--max-frames', type=int, default=50,
                       help='Maximum frames to process per video')
    parser.add_argument('--complexity', default='1_single',
                       help='Complexity level to process')
    parser.add_argument('--parts', nargs='+',
                       help='Specific parts to process (None = all)')
    parser.add_argument('--limit', type=int,
                       help='Limit number of parts to process')
