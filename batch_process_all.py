...
def process_single_part_orientation(part_code: str, orientation: str, 
                                   video_path: str, gt_path: str,
                                   vp: VideoProcessor, output_dirs: Dict,
    ...
    try:
        ...
        try:
            ...
            logger.info(f"  ✓ Generated {len(points_3d)} vs GT {len(gt_pcd.points)} points")
        ...
        # Save outputs
        try:
            pcd_file = output_dirs['point_clouds'] / f"{part_code}_{orientation}_reconstructed.ply"
        ...
        try:
            metrics_file = output_dirs['evaluations'] / f"{part_code}_{orientation}_metrics.json"
...
def process_all_parts(dataset_root: str, output_root: str, 
            ...
            try:
                ...
                for orientation, paths in part_data['orientations'].items():
                    ...
                    orient_result = process_single_part_orientation(
                        part_code, orientation,
                        paths['video'], paths['ground_truth'],
                        vp, output_dirs, logger
                    )
                    
                    part_result['orientations'][orientation] = orient_result
                
                # Aggregate orientation results
                successful = sum(1 for o in part_result['orientations'].values()
                               if o['status'] == 'success')
