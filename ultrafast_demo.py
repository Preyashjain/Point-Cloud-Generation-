...
def process_ultrafast(dataset_root: str, output_root: str):
    ...
    logger.info(
        f"Processing {len(all_parts)} parts × 3 levels = {len(all_parts) * 3} configs\n"
    )
    ...
    for part_code in all_parts:
        ...
        for complexity in ["1_single", "2_multiple", "3_stacked"]:
            ...
            try:
                ...
                logger.info(
                    f"[{config_num:2d}/78] {part_code} - {complexity:10s} - {orientation}...",
                    end=" ",
                )
    ...
    logger.info("=" * 70)
    logger.info(
        f"Completed: {results['completed']}/{len(all_parts) * 3} configurations"
    )
