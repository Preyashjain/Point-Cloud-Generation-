...
def verify_metrics_file(metrics_file, logger):
    ...
    try:
        ...
        return True, f"{metrics['points_generated']} pts, {metrics['matches_found']} matches, {metrics['frames_processed']} frames"
...
def compute_chamfer_distance(pred_points, gt_points, sample_size=5000):
    ...
    if len(gt_points) > sample_size:
        gt_points = gt_points[np.random.choice(len(gt_points), sample_size, replace=False)]
...
def main():
    ...
    output_root = Path('/Users/preyashjain/Downloads/Use_Case_2_Point_Cloud_Generation_dataset/outputs')
    dataset_root = Path('/Users/preyashjain/Downloads/Use_Case_2_Point_Cloud_Generation_dataset')
            ...
            try:
                ...
                if gt_file.exists():
                    ...
                    if gt_points is not None and len(pred_points) > 0:
                        ...
                        logger.info(f"  Ground Truth: {gt_count} points (ratio: {ratio:.2%})")
                        results[complexity]['comparisons'].append((ply_file.name, ratio))
            except Exception as e:
    ...
    ply_ok = sum(1 for level in results for ok in (results[level]['ply'][i][1] for i in range(len(results[level]['ply'])) if i < len(results[level]['ply'])))
    metrics_ok = sum(1 for level in results for ok in (results[level]['metrics'][i][1] for i in range(len(results[level]['metrics'])) if i < len(results[level]['metrics'])))
    ...
    if all_ok:
        ...
        logger.info("All generated files are valid and all three complexity levels are complete")
    else:
